...
## Phase 6D — Repository polish (20 September 2026)

### D-063 — The declared layout must match where outputs actually go
**Status:** Settled; two directories removed

`experiments/` and `reports/figures/` contained nothing but `.gitkeep`. Phase 1's
`experiment_plan.md` had specified experiment results into
`experiments/<EXP-ID>/results.json` and figures into `reports/figures/`, but the
implementation consolidated everything under `artifacts/` and never wrote to
either.

A reviewer cloning the repository finds two empty directories and reasonably
wonders what failed to generate. Worse, `config.py` declared three path constants
that nothing used, and `ensure_directories()` kept recreating them.

Both directories, their constants and their `.gitignore` rules were removed. A note
was added to `experiment_plan.md` pointing at `artifacts/`; **the plan itself is
left as written**, because it is a record of what was planned rather than a document
to retrofit. A test now fails if a directory containing only a placeholder
reappears.

**Generalisable:** a path constant that nothing writes to is dead structure, and
dead structure reads to a reviewer as an unfinished feature.

---

### D-064 — Screenshots are generated by a script, not taken by hand
**Status:** Settled

Screenshots are the part of a README most likely to go stale, because they are
captured once by hand and never again. `scripts/capture_screenshots.py` regenerates
all six from the running app in one command.

It drives headless Chrome rather than adding Playwright: a ~130 MB browser-automation
dependency for six images is not a trade worth making when Chrome is present on any
machine likely to review this.

**One defect found in the script itself.** The first capture of a session wrote an
11 KB blank page — Chrome had finished loading before Streamlit finished rendering.
The script now retries once with a longer virtual-time budget and validates every
file against a minimum rendered size, so a blank capture is reported rather than
silently committed. A screenshot tool that cannot tell "Chrome wrote a file" from
"Chrome captured the app" is not a tool.

---

### D-065 — The user's own files are not the agent's to delete
**Status:** Settled as a boundary

Three files sit in the working directory and not in the repository: a root-level
copy of the source workbook, a root-level copy of the official brief (both
byte-identical to the tracked canonical copies), and an empty `claude.md.txt`.

All three are already git-ignored, so they cost the submission nothing. Deleting
them would tidy the working directory — and would also delete files the user put
there, for reasons the agent does not know, with no way to undo it.

**They are listed as open items for the user's decision instead.** The repository
is the thing this phase was asked to make submission-ready; the user's working
directory is theirs.

---

### D-066 — A PII scanner that reports coincidences trains its reader to ignore it
**Status:** Settled after a false positive

The byte-wise scan of 858 real names and email addresses across every tracked file
returned one hit: `wwest`, inside `notebooks/03_segmentation_experiments.ipynb`.
Investigation showed it to be a five-character substring occurring by chance inside
the base64 of an embedded PNG.

The permanent check now matches **names at word boundaries** and emails exactly,
which returns zero across 190 files. The false positive is documented in the test's
own docstring so the next person does not have to re-derive it.

**Why this is worth a decision entry:** a scanner with a known-noisy signal is worse
than a stricter one, because the first real hit gets dismissed along with the
coincidences. The Phase 6A rule — a probe that cannot fail proves nothing — has a
counterpart here: a probe that fires on nothing real trains its reader to stop
looking.
